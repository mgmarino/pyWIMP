...
class FittingModelTime(BaseModel):
    ...
    def initialize(self, basevars, livetime):
        ...
        mean_list = [
                     ("73As", 11.103, 0.04, 0.04, 0.104, 1.414, 0.00, 80.30,        50,  2000), 
                     ("68Ge", 10.367, 0.10, 0.06, 0.108, 1.302, 0.00, 270.95,       0,  10000), 
                     ("71Ge", 10.367, 0.10, 0.04, 0.108, 1.302, 0.00, 11.26,        10,  10000), 
                     ("68Ga", 9.659,  0.10, 0.04, 0.112, 1.196, 0.00, 270.95,       0, 6000), 
                     ...
                     #("54Mn", 5.990,  0.10, 0.0,  0.139, 0.754, 0.00, 312.12,       80, 200),
        ...
        ga68_time_pdf = None
        ga68_fast_time_pdf = None
        ge_energy_pdf = None 
        ge_low_energy_pdf = None 
        for name, mean, mean_error, sigma_error, l_k_ratio, l_energy, l_energy_err, decay_time, decay_err, max_ampl in mean_list:

            input_dict = {}
            l_input_dict = {}
            input_dict['live_time'] = livetime
            l_input_dict['live_time'] = livetime
            if name == "68Ga": 
                input_dict['lifetime_value'] = ga68_time_pdf 
                input_dict['lifetime_err']   = None 
            else:
                input_dict['lifetime_value'] = decay_time 
                input_dict['lifetime_err']   = decay_err 
            

            if name == "71Ge": 
                input_dict['gamma_pdf'] = ge_energy_pdf 
            else:
                input_dict['mean_value']  = mean 
                input_dict['mean_error']  = mean_error 
                input_dict['sigma']       = get_sigma(mean*1e3)*1e-3 
                input_dict['sigma_error'] = sigma_error 
            

            agamma = gamma_line_model.GammaLineFactory.generate(
                        basevars,name, **input_dict)

            next_life = agamma.get_lifetime()

            l_input_dict['lifetime_value'] = next_life
            l_input_dict['lifetime_err']   = None
            if name == "68Ge": 
                ga68_time_pdf = next_life 
                ge_energy_pdf = agamma.get_energy_pdf()
            elif name == "68Ga": 
                l_input_dict['lifetime_value'] = ga68_time_pdf

            if name == "71Ge": 
                l_input_dict['gamma_pdf'] = ge_low_energy_pdf
                ga68_fast_time_pdf = next_life 
            else:
                l_input_dict['mean_value']  = l_energy 
                l_input_dict['mean_error']  = l_energy_err 
                l_input_dict['sigma']       = get_sigma(l_energy*1e3)*1e-3 
                l_input_dict['sigma_error'] = 0.0 
 
            agamma_low = gamma_line_model.GammaLineFactory.generate(basevars,name+"_l_line", **l_input_dict)
            if name == "68Ge": 
                ge_low_energy_pdf = agamma_low.get_energy_pdf()
        ...
        energy_exp_pdf = ROOT.RooExponential("energy_pdf_exp", 
                                             ...
                                             basevars.get_energy(),
                                             exp_constant_one)
        #exp_pdf_whole = ROOT.RooProdPdf("exp_pdf_whole", "exp_pdf_whole", energy_exp_pdf, time_pdf_flat)
        #low_erf_whole = ROOT.RooProdPdf("low_erf_whole", "low_erf_whole", low_erf_pdf, time_pdf_flat)
        exp_pdf_whole = ROOT.RooProdPdf("exp_pdf_whole", "exp_pdf_whole", energy_exp_pdf, time_pdf_exp)
